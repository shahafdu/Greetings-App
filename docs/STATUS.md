...
- [ ] **Phase 2 — native features** (one at a time, owner tests each on device):
  ...
  - [ ] Native calendar (community plugin) + runtime permission.
  - [ ] Native Google Sign-In (replaces the webview-breaking popup).
  - [ ] Biometric unlock + Android Keystore → auto-unlock the encrypted vault (no passphrase typing).
  - [ ] Verify WhatsApp opens via Android intent.
- [ ] **Phase 3 — build & test APK** on the owner's phone.
